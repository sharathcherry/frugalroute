...
4. **Local Inference on AMD MI300X**: Qwen2.5-32B-Instruct served via vLLM with ROCm 7.2.4 and Automatic Prefix Caching (RadixAttention). Free.
...
- **Inference**: vLLM + ROCm 7.2.4, Qwen2.5-32B-Instruct
...
## Real Results (MI300X, Qwen2.5-32B, 36-task eval)

| Metric | Value |
|---|---|
| Free local % | **61.1%** |
| Paid remote % | **38.9%** |
| Remote tokens used | ~14,054 tokens on sample |
| Calibration ECE | 0.303 → 0.002 (Platt) |
| GPU temp during inference | 53°C |
| GPU power during inference | 216W |
| vLLM model | Qwen2.5-32B-Instruct |
| ROCm version | 7.2.4 |
